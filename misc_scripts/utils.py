...
def ask_role_type(message):
    ...
    -------
    str
        The relation type (vocalist, backing_vocalist, performer, composer, arranger)
    """
...
def ask_song_ids():
    ...
    while True:
        ...
        if song_id == -1:
            song_name = input("Please input the song name exactly as it is:\n")
            song_artist = input("Please input the song artist exactly as it is:\n")

            song_ids.append([song_name, song_artist])
...
def ask_line_up(
    ...
    while True:
        ...
        role_type = ask_role_type("Please input the role type\n")
        ...
        if not artist_database[member_id]["line_ups"]:
            group_members.append(
                {"id": member_id, "line_up_id": -1, "role_type": role_type}
            )
            print(f"Adding {[member_id, -1, role_type]}")

        elif len(artist_database[member_id]["line_ups"]) == 1:
            group_members.append(
                {"id": member_id, "line_up_id": 0, "role_type": role_type}
            )
            print(f"Adding {[member_id, 0, role_type]}")
            ...
            group_members.append(
                {"id": member_id, "line_up_id": line_up_id, "role_type": role_type}
            )
            print(f"Adding {[member_id, line_up_id, role_type]}")
        print()
...
def add_new_artist_to_DB(
    ...
    if new_id not in artist_database:
        artist_database[new_id] = {
            "name": artist,
            "artist_amq_names": [artist],
            "artist_alt_names": [],
            "groups": [],
            "line_ups": [],
            "is_vocalist": vocalist,
            "is_performer": performer,
            "is_composer": composing,
            "is_arranger": arranging,
        }
    return new_id
    ...
    for anime_ann_id in song_database:
        ...
        for song in anime["songs"]:
            if artist_id in [aid["id"] for aid in song["vocalists"]] + [
                ...
                example_animes.add(anime["anime_expand_name"])
                break
...
def get_artist_id(
    ...
    vocalist : bool, optional
        Whether the artist is a vocalist, by default True
    performing : bool, optional
        Whether the artist is a performer, by default False
    composing : bool, optional
        Whether the artist is a composer, by default False
    arranging : bool, optional
        Whether the artist is an arranger, by default False
    partial_match : bool, optional
    ...
    if not exact_match:
        if partial_match:
            ...
            for id in artist_database.keys():
                flag = False
                for name in (
                    artist_database[id]["artist_amq_names"]
                    + artist_database[id]["artist_alt_names"]
                ):
        ...
        for id in artist_database.keys():
            if (
                artist
                in artist_database[id]["artist_amq_names"]
                + artist_database[id]["artist_alt_names"]
                and id not in ids
            ):
                ids.append(id)
    ...
    if not ids:
        if not not_exist_ok:
            ...
            # return -1
        new_id = add_new_artist_to_DB(
            artist_database, artist, vocalist, performing, composing, arranging
        )
...
def check_same_song(source_song, song):
    ...
    if song == source_song["ann_song_id"] or (
        type(song) == list
        and song[0] == source_song["song_name"]
        and song[1] == source_song["song_artist"]
    ):
        return True
    return False
